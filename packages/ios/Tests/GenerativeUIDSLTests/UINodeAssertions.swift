...
/// Load a JSON fixture from spec/test-fixtures/ using #file-based path resolution
func loadFixture(_ name: String, file: StaticString = #file) throws -> Data {
    let testDir = URL(fileURLWithPath: "\(file)").deletingLastPathComponent()
    let repoRoot = testDir
        .deletingLastPathComponent() // Tests/
        .deletingLastPathComponent() // ios/
        .deletingLastPathComponent() // packages/
        .deletingLastPathComponent() // generative-ui/ (repo root)
    let fixtureURL = repoRoot
        .appendingPathComponent("spec/test-fixtures")
